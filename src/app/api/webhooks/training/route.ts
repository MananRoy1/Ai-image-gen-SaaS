  ...
  try {
    ...
    //send a successful status email
      await resend.emails.send({
        from: "Pictoria-AI <roymanan111@gmail.com>",
        to: [userEmail],
        subject: "Model Training is completed!",
      ...
      // Delete the training model from supabase storage
      ...
      //Updating credits
        await supabaseAdmin.from("credits").update({model_training_count: (oldCredits?.model_training_count ?? 0) + 1}).eq("user_id", userId).single();
    }
