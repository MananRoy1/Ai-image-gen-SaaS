...
export async function getPresignedStoregeUrl(filePath: string) {
...
export async function deleteModel(
  ...
  if (model_version) {
    try {
      const res = await fetch(
        `https://api.replicate.com/v1/models/mananroy/${model_id}/versions/${model_version}`,
        {
          method: "DELETE",
          headers: {
  ...
  if (model_id) {
    try {
      const res = await fetch(
        `https://api.replicate.com/v1/models/mananroy/${model_id}`,
        {
          method: "DELETE",
          headers: {
            Authorization: `Bearer ${process.env.REPLICATE_API_TOKEN}`,
